...
public class manageConnections : Photon.MonoBehaviour {
	...
	void SelectServer()
	{
		...
		if (GUILayout.Button("Australia (Melbourne) ", GUILayout.Height(35)))
		{
			if (AppID != string.Empty)
			{
				PhotonNetwork.ConnectToMaster("app-au.exitgamescloud.com", Port, AppID, AppVersion);
				PlayAudioClip(a_Click, transform.position, 1.0f);
				StartCoroutine(Fade(LobbyState.MainMenu));
			}
		}
...
//		GUI.Box(new Rect(325, 10, 130, 35), "<color=black>Game Mode</color>");
	...
	private IEnumerator MoveToGameScene()
	{
		...
		Application.LoadLevel((string)PhotonNetwork.room.customProperties[PropertiesKeys.SceneNameKey]); 
	}
	// LOBBY EVENTS
